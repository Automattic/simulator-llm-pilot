...
- Add a `--rest-api-policy cleanup-delete-only` option that blocks REST content
  mutations before they reach the network. Setup and verification may only read
  posts, pages, categories, and tags; cleanup may read or delete them.
...
- Keep assert_element_* failures always enforced — including when REST
  verification passed (REST confirms server state; assertions check UI state,
  and one must not mask the other). The probe-vs-requirement distinction is
  handled at the tool level instead: build 32592 false-failed "Create a
  Scheduled Post" over an exploratory "Done" button probe, so the prompt and
  tool descriptions now reserve assert_element_* for conditions the test
  requires, steer probes to the unenforced wait_for_element, and explain how
  to clear a stale failing record with an honest opposite assertion once the
  UI reflects the correct end state.
